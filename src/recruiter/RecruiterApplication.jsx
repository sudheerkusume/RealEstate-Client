...
const RecruiterApplication = ({ onViewProfile }) => {
    ...
    useEffect(() => {
        ...
        axios.get("https://realestate-server-9xji.onrender.com/recruiter/applications", {
            headers: {
    ...
    const updateStatus = async (id, status) => {
        try {
            await axios.put(
                `https://realestate-server-9xji.onrender.com/recruiter/application-status/${id}`,
                { status },
                {
                    headers: {
                        Authorization: `Bearer ${token}`
            ...
            alert(`Status updated successfully to ${status} and email sent`);
        } catch (err) {
        ...
        <div className="container-fluid animate-fade-in">
            ...
            <div className="admin-view-header">
                <div className="admin-view-title">
                    <h3 className="fw-bold">Candidate Applications</h3>

                    <p className="text-muted">Manage your job applicants and their hiring status</p>
                </div>
                                ...
                                {filteredApplications.map(app => (
                                    <tr key={app._id}>
                                        ...
                                        <td>
                                            ...
                                            <div className="text-muted small">{app.jobId?.type || "Full Time"}</div>
                                        </td>
                                        <td>
                                        ...
                                        <td className="text-end">
                                            <div className="d-flex align-items-center justify-content-end gap-2">
                                                <button
                                                    className="btn btn-light-hover rounded-pill px-3 py-1 border small"
                                                    style={{ fontSize: '12px', fontWeight: '600' }}
                                                    onClick={() => onViewProfile(app.userId?._id || app.userId)}
                                                >
                                                    View Profile
                                                </button>
                                                <select
                                                    className="status-select-premium m-0"
